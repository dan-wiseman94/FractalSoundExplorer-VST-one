...
#include "RobotoMono-Medium.ttf.hpp"
...
struct Chunk {
  ...
  virtual bool onGetData(float* out1, float* out2, int frames){
...
//#else
void fractal_main(bool* running, Synth* synth) {
  ...
  sf::Shader shader;
  if (!shader.loadFromMemory(vert_glsl(), sf::Shader::Vertex)) {
  ...
  if (!shader.loadFromMemory(frag_glsl(), sf::Shader::Fragment)) {
  ...
  if(!font.loadFromMemory(RobotoMonoMedium, RobotoMonoMedium_len)){
